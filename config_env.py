...
def file_search_dirs():
    ...
    dirs = ['.', here,
            ...
            join(here, 'requirements_packages')]
...
def main():
    ...
    # install_python_dev()
    py_executable = create_environment(home_dir, clear=True)
    uwsgi_file = create_uwsgi_config(home_dir)
    create_admin(py_executable, uwsgi_file)
    create_nginx_config()
...
def create_uwsgi_config(home_dir):
    href = os.path.dirname(os.path.abspath(__file__))
    content = UWSGI_INI
    content = content.replace('__VIRTUAL_ENV__', os.path.abspath(home_dir))
    content = content.replace('__CAS_PATH__', os.path.join(href, "cas"))
    _file = os.path.join(href, "uwsgi.ini")
    writefile(_file, content)
    return _file


def check_pillow(py_executable):
    cmd = [py_executable, '-c', """
...
"""]
    ...
    stdout = proc.stdout
    if stdout.readline() != "":
        logger.warn('Pillow Error')


def create_admin(py_executable, uwsgi_file):
    content = CAS_ADMIN
    content = content.replace('__UWSGI__', uwsgi_file)
    content = content.replace('__PID__', "/tmp/cas.pid")
    content = content.replace('__PYTHON_BIN__', os.path.dirname(py_executable))
    href = os.path.dirname(os.path.abspath(__file__))
    _file = os.path.join(href, "cas-admin.py")
    writefile(_file, content)
    make_exe(_file)

    proc = subprocess.Popen(
        ["ln", "-s", _file, "/usr/local/bin/cas-admin"], stderr=subprocess.STDOUT, stdin=None, stdout=subprocess.PIPE)
    stdout = proc.stdout
    if stdout.readline() != "":
        logger.warn(stdout.readline())


def create_nginx_config():
    href = os.path.dirname(os.path.abspath(__file__))
    content = NGINX_HTTS_CONFIG
    content = content.replace('__PATH__', os.path.join(href, "cas"))
    writefile(os.path.join(href, "config", "cas_https"), content)

    content = NGINX_CONFIG
    content = content.replace('__PATH__', os.path.join(href, "cas"))
    writefile(os.path.join(href, "config", "cas_http"), content)
...
def create_environment(home_dir, site_packages=False, clear=False,
    ...
    install_packages(py_executable)
    check_pillow(py_executable)
    create_database(py_executable)
...
##file uwisg.ini
UWSGI_INI = convert("""

""")

##file cas-admin.py
CAS_ADMIN = convert("""

""")

NGINX_HTTS_CONFIG = convert("""

""")

NGINX_CONFIG = convert("""

""")
...
packages = [
    ...
    'python-dateutil-2.4.1.tar.gz',
    'python-mimeparse-0.1.4.tar.gz',
    'django-grappelli-2.6.3.tar.gz',
    'django-extensions-1.5.0.tar.gz',
    'oauthlib-0.7.2.tar.gz',
    'DjangoCaptcha-0.2.13.tar.gz',
    'django-braces-1.4.0.tar.gz',
    'pexpect-3.3.tar.gz',
    ...
    'django-compresshtml-0.1.1.tar.gz',
    'python-ldap-2.4.19.tar.gz',
    'django-ldapdb-0.3.2.tar.gz'
]
...
def install_packages(py_executable):
    href = os.path.dirname(os.path.abspath(__file__))
    logger.notify('Install Requirements Packages')
    easy_install_script = 'easy_install'
    if sys.platform == 'win32':
        easy_install_script = 'easy_install-script.py'
    easy_install_script = join(os.path.dirname(py_executable), easy_install_script)

    def package_search_dirs(name):
        here = join(os.path.dirname(os.path.abspath(__file__)), "requirements_packages")
        return join(here, name) if os.path.exists(join(here, name)) else None

    for package in packages:
        package_dir = package_search_dirs(package)
        if package_dir:
            logger.start_progress('Installing %s...' % package)
            logger.indent += 2
            try:
                call_subprocess([easy_install_script, os.path.join(href, "requirements_packages", package)],
                                show_stdout=False)
            finally:
                logger.indent -= 2
                logger.end_progress()
        else:
            logger.fatal("Error:%s不存在" % package)
            sys.exit(1)


def create_database(py_executable):
    href = os.path.dirname(os.path.abspath(__file__))
    href = join(href, "cas")
    logger.start_progress('Creating database ...')
    logger.indent += 2

    try:
        call_subprocess([py_executable, join(href, "manage.py"), "syncdb", "--noinput"],
                        show_stdout=False)
    finally:
        logger.indent -= 2
        logger.end_progress()

    logger.start_progress('Creating superusers ...')
    try:
        call_subprocess([py_executable, join(href, "manage.py"), "loaddata", join(href, "superusers.json")],
                        show_stdout=False)
    finally:
        logger.indent -= 2
        logger.end_progress()
    # print proc.stdout.readlines()
    #logger.notify("用户名:admin")
    #logger.notify("密码:123456")

def install_python_dev():
    logger.start_progress('install python_dev ...')
    logger.indent += 2
    try:
        child = pexpect.spawn("apt-get install python-dev libjpeg8-dev  libpng12-dev libfreetype6-dev zlib1g-dev libxml2-dev libxslt1-dev libldap2-dev libsasl2-dev",)
        print child.expect(['.*[Y/n]*', pexpect.EOF, pexpect.TIMEOUT])
        child.sendline("Y")
        child.interact()
    finally:
        logger.indent -= 2
    ...
    logger.start_progress('install image ...')
