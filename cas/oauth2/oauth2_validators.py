...
from .models import Grant, AccessToken, RefreshToken, get_application_model, UserOpenID
...
class OAuth2Validator(RequestValidator):
    ...
    def save_bearer_token(self, token, request, *args, **kwargs):
        ...
        AccessToken.objects.filter(user=request.user, application=request.client, valid=True).update(valid=False)
        ...
        access_token.save()
        try:
            UserOpenID.objects.get(user=request.user, application=request.client)
        except ObjectDoesNotExist:
            UserOpenID(user=request.user, application=request.client).save()
